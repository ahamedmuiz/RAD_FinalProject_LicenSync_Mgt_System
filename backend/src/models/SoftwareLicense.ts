...
const softwareLicenseSchema: Schema = new Schema(
  {
    clientId: { type: Schema.Types.ObjectId, ref: 'ClientCompany', required: true },
    softwareName: { type: String, required: true },
    vendor: { type: String, required: true },
    seatCount: { type: Number, required: true, min: 1 },
    licenseKey: { type: String, required: true }, // In a full prod app, we'd encrypt/decrypt this
...
const SoftwareLicense = mongoose.model<ISoftwareLicense>('SoftwareLicense', softwareLicenseSchema);
